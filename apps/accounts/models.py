...
class User(AbstractUser):
    ...
    # Для персонала с лицензируемой практикой (врачи и т.п.) — справочно,
    # ничего не блокирует по истечении (нет интеграции с реестром
    # лицензирующего органа). Правится вручную через admin, тот же
    # принцип, что Patient.loyalty_points — простое поле, не отдельная
    # система учёта документов.
    license_expires_at = models.DateField(
        null=True, blank=True, verbose_name="Срок действия лицензии",
    )
